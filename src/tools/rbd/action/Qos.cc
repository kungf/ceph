				    ...
				    uint64_t bps_burst, uint64_t bps_avg)
{
  ...
  r = image.qos_get(&old_iops_burst, &old_iops_avg, &old_bps_burst, &old_bps_avg);
  if (r < 0) {
  ...
  return image.qos_set(iops_burst, iops_avg, bps_burst, bps_avg);
}

int do_set_iops(librbd::Image& image, uint64_t burst, uint64_t avg)
{
  return do_qos_set(image, burst, avg, 0, 0);
}

int do_clear_iops(librbd::Image& image)
{
  return do_qos_set(image, UINT64_MAX, UINT64_MAX, 0, 0);
}
